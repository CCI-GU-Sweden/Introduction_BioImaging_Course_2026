...
Now navigate to the directory where you want to store the course files:
```
...
```
Where ```python==3.11``` means we want a specific version (3.11) of python in this environment
...
You may also install multiple packages with one line of code. Foe example, to install pandas, numpy and pillow:
```
...
Below is additional information about environments that may be useful. It is included below but is not needed for Day 5.
